...
class SubscriptionController extends Controller
{
    ...
    public function join(Request $request)
    {
        $request->validate([
            ...
            'days' => 'nullable|integer|min:1', // For custom daily duration
        ]);
        ...
        if (Str::lower($plan->type) === 'trial') {
            ...
            if ($hasTrial) {
                return response()->json(['message' => 'You have already used your trial.'], 403);
            }
    ...
    private function generatePayment($user, $plan, $request)
    {
        ...
        if ($plan->duration_unit === 'months') $totalDays *= 30; // approximation for display
        ...
        // Meta data for webhook to store temporary info
        $params = [
            'transaction_details' => [
                'order_id' => $orderId,
